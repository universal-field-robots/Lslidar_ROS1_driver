...
#include <lslidar_ch128x1_msgs/LslidarChSweep.h>
#include <lslidar_ch128x1_msgs/LslidarChLayer.h>
#include <math.h>
...
private:
   ...
   // int checkPacketValidity(const RawPacket* packet);
    ...
    void packetCallback(const lslidar_ch128x1_msgs::LslidarChPacketConstPtr& msg);
    // Publish data
    ...
    double rawAzimuthToDouble(const uint16_t& raw_azimuth) {
     ...
     double verticalLineToAngle(const uint16_t& vertical_line) {
       ...
       // return static_cast<double>(vertical_line)*0.25-17;
                return static_cast<double>((big_angle[vertical_line/4]) * DEG_TO_RAD);
    ...
    lslidar_ch128x1_msgs::LslidarChScanPtr sweep_data;
