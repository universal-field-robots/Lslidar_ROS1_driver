...
{
static const size_t ms_packet_size = sizeof(lslidar_c16_msgs::LslidarC16MsPacket().data);
static const size_t dif_packet_size = sizeof(lslidar_c16_msgs::LslidarC16DifPacket().data);
...
int InputSocket::getMsPacket(lslidar_c16_msgs::LslidarC16MsPacket* pkt, const double time_offset)
{
 ...
 // while (true)
  {
    ...
    ssize_t nbytes = recvfrom(sockfd_, &pkt->data[0], ms_packet_size, 0, (sockaddr*)&sender_address, &sender_address_len);
    ...
    else if ((size_t)nbytes == ms_packet_size)
    {
  ...
  return 0;
    ...
    int InputSocket::getDifPacket(lslidar_c16_msgs::LslidarC16DifPacket* pkt, const double time_offset)
    {
        double time1 = ros::Time::now().toSec();
        struct pollfd fds[1];
        fds[0].fd = sockfd_;
            ...
            // while (true)
        {
            ...
            ssize_t nbytes = recvfrom(sockfd_, &pkt->data[0], dif_packet_size, 0, (sockaddr*)&sender_address, &sender_address_len);
            ...
            else if ((size_t)nbytes == dif_packet_size)
            {
                if (devip_str_ != "" && sender_address.sin_addr.s_addr != devip_.s_addr)
                    continue;
                else
...
/** @brief Get one lslidar packet. */
int InputPCAP::getMsPacket(lslidar_c16_msgs::LslidarC16MsPacket* pkt, const double time_offset)
{
 ...
 // while (flag == 1)
  while (true)
  {
    ...
    if ((res = pcap_next_ex(pcap_, &header, &pkt_data)) >= 0)
    {
      ...
      memcpy(&pkt->data[0], pkt_data + 42, ms_packet_size);
  ...
  if (flag == 0)
  {
    ...
    int InputPCAP::getDifPacket(lslidar_c16_msgs::LslidarC16DifPacket* pkt, const double time_offset)
    {
        struct pcap_pkthdr* header;
        const u_char* pkt_data;
