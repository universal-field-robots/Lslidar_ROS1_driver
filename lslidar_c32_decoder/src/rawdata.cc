...
void RawData::loadConfigFile(ros::NodeHandle node, ros::NodeHandle private_nh)
{
  ...
  difop_sub_ = node.subscribe("lslidar_packet_difop_c32", 10, &RawData::processDifop, (RawData*)this);
}
...
void RawData::unpack(const lslidar_c32_msgs::LslidarC32Packet& pkt, VPointCloud::Ptr pointcloud, lslidar_c32_msgs::LslidarC32SweepPtr& sweep_data,  int Packet_num)
{
  ...
  if(config_vert_angle){
	...
	int ring;
	float vert1[32][2] = { 0.0 };
	float vert2[32][2] = { 0.0 };
	int vert_num[32] = { 0 };
	 ...
	 for (int firing = 0, k = 0; firing < LSC32_FIRINGS_PER_BLOCK; firing++) 
    {
      for (int dsr = 0; dsr < LSC32_SCANS_PER_FIRING; dsr++, k += RAW_SCAN_SIZE)  // 32  3
      {
        ...
        if (distance2 > max_distance_ || distance2 < min_distance_)
        {
            ...
            pointcloud->at(this->block_num, ring) = point;
        }
        else
        {
            //均匀1度校准两列
			...
			if (coordinate_opt) {
				point.x = distance2 * cos_scan_altitude_caliration[dsr]  * cosf(rotation_azimuth) + (LSC32_DISTANCE_TOFFSET * cosf(rotation_azimuth_offset)) * DISTANCE_RESOLUTION;
				point.y = -(distance2 * cos_scan_altitude_caliration[dsr]  * sinf(rotation_azimuth) + (LSC32_DISTANCE_TOFFSET * sinf(rotation_azimuth_offset)) * DISTANCE_RESOLUTION);
			...
			point.ring = ring;
			point.time = (double)point_time/1000000000 ;
            pointcloud->at(this->block_num, ring) = point;
			
			//Save to LaserScan display
			 sweep_data->scans[ring].points.push_back(
                            lslidar_c32_msgs::LslidarC32Point());
							
			lslidar_c32_msgs::LslidarC32Point& new_point =
					sweep_data->scans[ring].points[
					sweep_data->scans[ring].points.size()-1];
