...
#include <lslidar_ch_driver/lslidar_ch16_driver_nodelet.h>
...
{
  // start the driver
  lslidar_ch16_driver.reset(
      new LslidarChDriver(getNodeHandle(), getPrivateNodeHandle()));
  if (!lslidar_ch16_driver->initialize()) {
...
void LslidarChDriverNodelet::devicePoll()
{
  while(ros::ok()) {
    ...
    running = lslidar_ch16_driver->polling();
...
PLUGINLIB_EXPORT_CLASS(
                        lslidar_ch_driver::LslidarChDriverNodelet, nodelet::Nodelet);
