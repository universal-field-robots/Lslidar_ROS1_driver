...
#include <lslidar_ch_driver/lslidar_ch16_driver.h>
...
bool LslidarChDriver::createRosIO() {
  ...
  // packet rate.
  const double diag_freq = 32*20000.0 / 171;
  ...
  ROS_INFO("expected packet frequency: %.3f (Hz)", diag_freq);
    ...
    // Output
    packet_pub = nh.advertise<lslidar_ch16_msgs::LslidarChPacket>(
                "lslidar_packet_ch16", 100);
    double packet_rate = 3571;
    ...
    if(dump_file != ""){
        ...
        msop_input_.reset(new lslidar_ch_driver::InoutSocket(pnh, msop_udp_port));
        difop_input_.reset(new lslidar_ch_driver::InoutSocket(pnh, difop_udp_port));
    }
...
bool LslidarChDriver::initialize() {
    ...
    ROS_INFO("Initialised lslidar ch without error");
    return true;
}
...
bool LslidarChDriver::polling()
{
    ...
    lslidar_ch16_msgs::LslidarChPacketPtr packet(
                new lslidar_ch16_msgs::LslidarChPacket());
...
void LslidarChDriver::difopPoll() {
    lslidar_ch16_msgs::LslidarChPacketPtr difop_packet(new lslidar_ch16_msgs::LslidarChPacket());
    two_bytes angle_1, angle_2, angle_3, angle_4;
    while(ros::ok()){
        int rc = difop_input_->getPacket(difop_packet);
        if(0 == rc){
            getFPGA_GPSTimeStamp(difop_packet);
            angle_1.bytes[0] = difop_packet->data[663];
            angle_1.bytes[1] = difop_packet->data[662];
            this->prism_angle[0] = angle_1.value * 0.01;

            angle_2.bytes[0] = difop_packet->data[665];
            angle_2.bytes[1] = difop_packet->data[664];
            this->prism_angle[1] = angle_2.value * 0.01;

            angle_3.bytes[0] = difop_packet->data[667];
            angle_3.bytes[1] = difop_packet->data[666];
            this->prism_angle[2] = angle_3.value * 0.01;

            angle_4.bytes[0] = difop_packet->data[669];
            angle_4.bytes[1] = difop_packet->data[668];
...
void LslidarChDriver::getFPGA_GPSTimeStamp(lslidar_ch16_msgs::LslidarChPacketPtr &packet)
{
