...
void LslidarN301Decoder::publishScan()
{
	...
	int scan_num = point_num*(end_angle - start_angle)/360;
    ...
    for(uint16_t j=0; j < scan_sizes; j++)
    {
      for(uint16_t i = 0; i < sweep_data->scans[j].points.size(); i++)
      {
        ...
        if (point_idx >= start_num && point_idx < end_num)
		{
			point_idx = point_idx - start_num;
			scan->ranges[scan_num - 1-point_idx] = sweep_data->scans[j].points[i].distance;
			scan->intensities[scan_num - 1-point_idx] = sweep_data->scans[j].points[i].intensity;
		}
      }
    }
