...
namespace lslidar_ch_driver {
    ...
    bool LslidarChDriver::createRosIO() {
        ...
        // packet rate.
        const double diag_freq = 64 * 20000.0 / 200;
       ...
       // ROS_INFO("expected frequency: %.3f (Hz)", diag_freq);
        ...
        // Output
        packet_pub = nh.advertise<lslidar_ch64w_msgs::LslidarChPacket>(
                "lslidar_packet_64w", 100);
    ...
    bool LslidarChDriver::polling() {
        ...
        if (time_synchronization_) {
            ...
            lslidar_ch64w_msgs::LslidarChPacket pkt = *packet;
    ...
    void LslidarChDriver::getFPGA_GPSTimeStamp(lslidar_ch64w_msgs::LslidarChPacketPtr &packet) {
        ...
        if (head2[0] == 0xA5 && head2[1] == 0xFF) {
            if (head2[2] == 0x00 && head2[3] == 0x5A) {
                /*    this->packetTimeStamp[4] = packet->data[57];
                    this->packetTimeStamp[5] = packet->data[56];
                    this->packetTimeStamp[6] = packet->data[55];*/
