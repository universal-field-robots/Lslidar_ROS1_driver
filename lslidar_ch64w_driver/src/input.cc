...
namespace lslidar_ch_driver {
    static const size_t packet_size = sizeof(lslidar_ch64w_msgs::LslidarChPacket().data);
////////////////////////////////////////////////////////////////////////
...
/** @brief destructor */
    ...
    int InputPCAP::getPacket(lslidar_ch64w_msgs::LslidarChPacketPtr &packet) {
        ...
        while(flag == 1){
            ...
            if((res = pcap_next_ex(pcap_, &header, &pkt_data)) >= 0){
                ...
                if(packet->data[0] == 0xA5 && packet->data[1] ==0xFF && packet->data[2] ==0x00 && packet->data[3] ==0x5A){
                    int rpm =(packet->data[8] << 8) | pkt_data[9];
                    int mode = 1;
                    if((packet->data[45] == 0x08 && packet->data[46] == 0x02 && packet->data[47] >= 0x09) ||
                            (packet->data[45] >0x08) ||
                            (packet->data[45] == 0x08 && packet->data[46] > 0x02)){
                        if(packet->data[300] != 0x01 && packet->data[300] != 0x02){
                            mode = 0;
                        }
                    }
