...
void LslidarChDecoder::decodePacket(const RawPacket* packet) {
    ...
    for (size_t point_idx = 0; point_idx < POINTS_PER_PACKET; point_idx++) {
         ...
         firings[point_idx].vertical_line=packet->points[point_idx].vertical_line;
          ...
          TwoBytes point_distance;
          point_distance.bytes[0] = packet->points[point_idx].distance_2;
          point_distance.bytes[1] = packet->points[point_idx].distance_1;
...
int LslidarChDecoder::checkPacketValidity(const RawPacket* packet) {
    ...
    if ((packet->points[blk_idx].vertical_line==0xff)&&(packet->points[blk_idx].azimuth_1 ==0xaa)&&(packet->points[blk_idx].azimuth_2 ==0xbb)&&(packet->points[blk_idx].distance_1 ==0xcc)
...
void LslidarChDecoder::packetCallback(
  ...
  for (size_t point_idx = 0; point_idx < POINTS_PER_PACKET; point_idx++) {
    ...
    z_sin_altitude=sin_scan_laser_altitude[firings[point_idx].vertical_line/4]+2*cos_azimuth_half*sin_scan_mirror_altitude[firings[point_idx].vertical_line%4];
    //printf("a1=%d,a2=%d \n",firings[point_idx].vertical_line/4,firings[point_idx].vertical_line%4);
    ...
    //  double time = packet_start_time +FIRING_TOFFSET*point_idx + DSR_TOFFSET*point_idx;
    double time =0.0;
