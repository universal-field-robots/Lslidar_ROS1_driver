...
int LslidarChDecoder::ConvertCoordinate(struct Firing lidardata)
{
  ...
  unsigned int line_angle = lidardata.vertical_line >> 4;
  unsigned int mask = ~(~0 << 4);
  unsigned int vert_angle_select = line_angle & mask;
  double vert_angle;

  if(0x0 == vert_angle_select){
    vert_angle = vertical_angle[0];
  }else if(0x1 == vert_angle_select){
    vert_angle = vertical_angle[1];
  }else if(0x2 == vert_angle_select){
    vert_angle = vertical_angle[2];
  }else if(0x3 == vert_angle_select){
    vert_angle = vertical_angle[3];
  }
  vert_angle = vert_angle/1000.f * DEG_TO_RAD;
