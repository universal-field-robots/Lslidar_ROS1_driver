...
lslidarDriver::lslidarDriver(ros::NodeHandle node, ros::NodeHandle private_nh)
{
    ...
    private_nh.param("output_packets_topic", output_packets_topic, std::string("lslidar_packet_c32"));
    msop_output_ = node.advertise<lslidar_c32_msgs::LslidarC32ScanUnified>(output_packets_topic, 10);

    std::string output_difop_topic;
    private_nh.param("output_difop_topic", output_difop_topic, std::string("lslidar_packet_difop_c32"));
...
bool lslidarDriver::poll(void)
{
	...
	/*uint64_t timeStamp_now = GPSCountingTS * 1000000000 + packet_timestamp;
	if((timeStamp_now - last_timestamp > 120000000) || timeStamp_now <= last_timestamp)
		ROS_WARN("timeStamp: %ld, last_timestamp: %ld", timeStamp_now, last_timestamp);
